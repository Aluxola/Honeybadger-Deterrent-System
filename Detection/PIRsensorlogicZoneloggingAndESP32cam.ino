...
// === ESP32-CAM Trigger URL ===
// Replace with your friend's ESP32-CAM IP address
const char* camTriggerURL = "http://192.168.181.25/capture";  // <- her camera’s IP address
...
void loop() {
  ...
  if ((now - pir1_trigger_time <= trigger_window && now - pir2_trigger_time <= trigger_window) &&
    ...
    triggerCamera();  // 🔥 Trigger the camera
...
void triggerCamera() {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin(camTriggerURL);
    int httpCode = http.GET();

    if (httpCode > 0) {
      Serial.println("✅ Camera triggered successfully");
    } else {
      Serial.println("❌ Failed to trigger camera");
    }

    http.end();
  } else {
    Serial.println("❌ Not connected to WiFi — can't trigger camera");
  }
}
